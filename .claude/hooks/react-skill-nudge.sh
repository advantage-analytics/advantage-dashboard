...
print(json.dumps({
    ...
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            ...
            "reference — read it first (it is ~200 lines), then load only the "
            "`reference/` file its routing table points you to: `foundations.md` for any "
            "token, `components.md`, `chrome.md`, `tables.md`, `primitives.md`, "
            "`settings.md`, `home-recipes.md`, `empty-and-loading.md`, `focus.md`. "
            "Primary buttons come from `advButton()` in src/lib/ui/adv-button.ts.\n"
