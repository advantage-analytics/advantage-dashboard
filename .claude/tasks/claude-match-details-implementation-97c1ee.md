...
- **done when:**
  - [ ] `viewer-side.ts` exports `playerSeat(match, playerIds): "player1" | "player2" | null` (seat one if `player1_id ∈ ids`, else seat two if `player2_id ∈ ids`, else null — never inferred from a null column) and `viewerSide()` delegates to it before its uploader clause, so `performance-server.ts` and `recent-activity.tsx` behave exactly as before
  - [ ] `match-stats-server.ts` exports a pure `ownSeatRows(matches, stats, playerIds): PlayerStatRow[]` that keeps only each match's own-seat stat row via `playerSeat` (both ids in the set → exactly the seat-one row; a stat row for a match not in `matches` → dropped) and strips `is_player1` from the output; `fetchPlayerStatRows` fetches matches with `.or("player1_id.in.(…),player2_id.in.(…)")` selecting `id, date, player1_id, player2_id`, fetches stat rows with `is_player1` and NO `.eq("is_player1", …)`, and returns `ownSeatRows(...)`
  - [ ] `getMatchKpiHistory(playerIds, matchId)` — the `matchDate` parameter and the bare `{ match_id, date }` anchor fallback are gone; `resolveKpiHistory` in `match-detail-server.ts` calls the new signature and its `Pick` drops `"date"`; `getPlayerAverageStats` keeps its signature and returns the same 18 fields
  - [ ] `npx playwright test tests/own-seat-rows.spec.ts` passes and pins at least: seat-two match keeps its `is_player1 = false` row (the bug), both-null row dropped, stranger-in-seat-one with null seat two dropped, both ids in the set → one seat-one row, unknown-match stat row dropped, `date` carried with null preserved, no `is_player1` key on output; `tests/match-kpi-history.spec.ts` passes with only its line-~205 comment rewritten
  - [ ] `npx tsc --noEmit` exits 0 and `npm run lint` reports 0 errors
- **notes:** Approved plan: `~/.claude/plans/elegant-shimmying-boot.md`. Unblocks T5 (stash `4de0791`), whose sparkline could end on an older match because the helper only fetched seat-one matches and then manufactured an empty anchor. Fix the source, not the symptom; do NOT add a component-side "last point equals headline" gate (break points saved rounds twice on the series path). RLS argument for the reviewer: policy applies before `.or()` narrows, both reads share `visible_match_ids()`, the opponent's already-readable stat row is discarded server-side in `ownSeatRows`. Doc comments in `fetchPlayerStatRows`, `getMatchKpiHistory` and `buildKpiHistory` currently describe the seat-one limitation as deliberate — rewrite them to say why both seats and why no fallback, in the repo's why-not-what voice.
