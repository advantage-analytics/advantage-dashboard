...
- **done when:**
  - [ ] `film-court-ball.tsx` renders the dot alone: the `<svg data-film-ball-tail>`, `MAX_TAIL_SEGMENTS`, `TAIL_MIN_ALPHA`, `TAIL_MAX_ALPHA`, `segmentIndexes`, `segmentRefs` and the file's own `usePrefersReducedMotion` (plus its `reducedMotionNow` import) are gone; `draw()` still writes only `left`/`top`/`visibility` from `ballAt`, and the rAF / `seeked` effect is byte-identical
  - [ ] `film-ball.ts` drops `BallAt.tail` and `BALL_TAIL_SECONDS` (`ballAt` returns the interpolated head; `BallAt` may collapse into `BallPoint`), and `tests/film-ball.spec.ts` drops exactly "the tail window is 0.4s" and "the tail is the samples in the window, oldest first, then the head" — every other test in it passes unchanged
  ...
  - [ ] `film-fullscreen.tsx`, `use-ball-paths.ts` and the derivation under `src/lib/services/splitstep/` are not in the diff; `npm run lint` and `npm run typecheck` pass
