...
## T38 · Remove the vertical tick bars from the sharing-terms rows

- **status:** later
- **files:** `src/components/join/join-terms.tsx` (the private `Tick` component);
  callers on `/join/[token]` (`src/components/join/join-forms.tsx` → `JoinReady`),
  the sign-up state of that page, `/invitations/[inviteId]`, and onboarding step
  zero; separately `src/app/onboarding/onboarding-flow.tsx` (its guardian step
  draws a similar 2px tick-bar — in scope only if you say so)
- **done when:**
  - [ ] The 2 × 12 px vertical tick no longer renders before sharing-terms rows on `/join/[token]` in its `ready` state
  - [ ] Every other surface `JoinSharingTerms` renders on is checked and changed or deliberately exempted — the sign-up state, `/invitations/[inviteId]`, onboarding step zero
  - [ ] The two columns ("Your coaches will see" / "Stays yours") remain visually distinguishable without the blue/ink tick carrying that job alone
  - [ ] The onboarding guardian step's own tick-bar is either changed to match or explicitly left, and which was chosen is stated
- **notes:** Author's words, verbatim: "Remove the vertical bars from the
  'Join your program' page." Carried over from `work/join-page-vertical-bars/`,
  a feature-pipeline workspace that was scaffolded on 2026-09-06 and never
  started — no brief, design, plan or build output was ever written. The
  workspace was removed and its research preserved here rather than deleted:
  the trace above (four surfaces, the private `Tick`, the separate onboarding
  bar) was the only content it held, and it is worth more in the queue than in
  an empty scaffold. Re-scaffold with `/feature-new` if it turns out to want the
  full pipeline; the trace suggests it does not.
