...
- **notes:** Found by `/pr-check`. T3 routed `match-summary-row` through the shared `tiebreakOf` and dropped the local `set.tiebreak && mine === 7 && theirs === 6` check in the process. `tiebreakOf` already refuses a tiebreak filed against the set's winner, and documents why ("a misfiled value renders nothing rather than a plausible lie") — this is the same argument applied to the set score, so the guard belongs beside it.
  **Found live at HEAD by `/pr-check`, 2026-08-25 — the bug is worse than "a missing guard".** `tiebreakOf` uses `?? null`, and `0` is not nullish, so a stored `0` returns `0` and `<ScoreLine>` (which gates on `tiebreak !== null`) renders `6-4` with a superscript zero. The pre-branch `matches-list-types.ts` guarded this with `(… ?? 0) > 0`; T3 removed that path. **But restoring `> 0` would be wrong too:** a tiebreak won 7-0 in points is a real score, so `7-6⁰` is legitimate and `> 0` would hide it. That is the proof the guard must be on the SET SHAPE — which set scores a tiebreak can decide — and not on the value, which is exactly what this task is blocked on establishing. Confirmed independently by `code-review` and `pipeline-guardrails-reviewer`; both note it fires on every `<ScoreLine>` consumer (matches list, Team Home rows, search palette) and on the match-detail scoreboard.
