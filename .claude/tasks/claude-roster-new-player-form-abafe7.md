...
- **done when:**
  - [ ] The new migration `create or replace`s `public.program_former_players(p_program_id uuid)` with the same signature and `returns table (profile_id uuid, display_name text, email text, archived_at timestamptz, match_count bigint)`, still `language sql stable security definer set search_path = ''`, and its body contains no correlated scalar subquery: `match_count` comes from one `left join public.matches m` (or one pre-aggregated CTE/lateral over `matches` grouped by profile) with `count(m.id)` and a `group by` over the `program_players` columns, so a profile with no matches reads `0`, not `null`, and each match is counted once even when both its player columns hit the same profile
  - [ ] The join predicate folds both id eras the way `canonicalRosterIds` does for a live row: a match counts when `m.player1_id` or `m.player2_id` equals `pp.id` **or** equals `pp.claimed_by_user_id` (`program_roster_full` arm 1 maps `user_id = pp.claimed_by_user_id` onto `player_id = pp.id`; `roster-ids.ts:66-68` is the TS half). An unclaimed profile (`claimed_by_user_id is null`) counts only `pp.id` matches — the predicate is null-safe, not `= null`. The file's header comment says why in one short paragraph and names `src/lib/data/roster-ids.ts`
  - [ ] Every T6 guarantee reads the same in the new body: rows filtered on `pp.program_id = p_program_id and pp.archived_at is not null and pp.merged_into_id is null and p_program_id in (select public.user_program_ids())`, the join scoped with `m.program_id = pp.program_id` so a count never crosses programs, `order by pp.archived_at desc`, and the revoke-from-`public`/-`anon` + grant-to-`authenticated` lines re-stated for the function
  - [ ] The migration touches nothing else: `20260913031528_restore_program_player.sql` is byte-identical in the diff, and the new file defines no other function and does not drop/re-add `program_audit_log_action_check`
  - [ ] Applied live via the Supabase MCP `apply_migration`, and verified with `execute_sql`: `pg_get_functiondef('public.program_former_players(uuid)'::regprocedure)` shows the new body (mentions `claimed_by_user_id`, no `(select count(*)` inside the select list); `information_schema.routine_privileges` still shows `authenticated` only; and a read-only probe on ZZ Test Program (`edaf1aa0…`, 13 archived rows, ≥1 claimed) returns the same `profile_id` set as before with every `match_count` ≥ its previous value — the before/after pairs are quoted in the runner's log entry
- **notes:** Fixes two confirmed `/pr-check` findings on T6's `program_former_players`: (1) the correlated `match_count` subquery runs once per archived row — N+1; (2) it only matched `pp.id`, so a claimed-then-archived player's PRE-claim matches (recorded under their raw auth uid, per `docs/ui-revamp-guardrails.md` §2 — claiming deliberately does not re-attribute them) were missing from the Add Player dialog's "removed with N matches" note, which drives a real restore-vs-add-as-new decision. **Never edit the applied `20260913031528` file** — forward-only. **Schema truth is the live database** (AGENTS.md): before writing, confirm the current live definition via `execute_sql` on `pg_get_functiondef` — if it already differs from the file in the folder, replace what is live, not what the folder says. Load `supabase:supabase-postgres-best-practices` before writing SQL. `create or replace` preserves existing grants, but re-state them so the file reads whole. Do not add a `claimed_by_user_id`/`is_claimed` output column here — T7's `FormerPlayer` type consumes the RPC's current shape and T8 was told to gate on the RPC rather than a loader field; widening the return table is a separate task. Never hand-format `supabase/migrations/` (`.prettierignore`). The Supabase MCP in the runner's session must be authorized; if it isn't, the task is blocked, not done. Independent of T7–T9 (no `needs:`). Heads-up from the T7 log entry: `tests/teams-management.spec.ts:149` fails on the clean tree because of a live `update_program_settings` change made from another worktree — if the mechanical gate fails on that spec alone, that is the pre-existing regression, not this diff; it still needs its own fix before any task can land.
