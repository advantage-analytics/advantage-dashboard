...
- **done when:**
  - [ ] `stored` is computed as `nextSaved ? !error || error.code === "23505" : !error` — no `data.length` / row-count check remains in `handleToggleSaved` for either branch (grep evidence: `data.length` does not appear in the function).
  - [ ] An unsave whose delete returns `{ data: [], error: null }` (zero rows — the current mock's shape) is treated as landed: the optimistic `saved: false` is kept, not reverted. A save whose insert returns `error.code === "23505"` is likewise kept; a save whose insert returns any other error (e.g. an RLS refusal) still reverts.
  - [ ] The doc comment above `handleToggleSaved` is rewritten to explain that both "already in the desired state" outcomes (a zero-row delete, a `23505` insert conflict) count as landed because the toggle asks for a desired state rather than proof of a fresh row, and drops the false claim that "the next toggle repairs it" for the 23505 path.
  - [ ] `tests/fixtures/supabase-client-browser-mock.ts`'s header comment no longer describes zero rows as "the shape the component treats as not-stored and reverts"; it instead documents that both chains resolving `{ data: [], error: null }` now count as landed, and the mock gains a way to make an insert resolve a non-`23505` error so a revert is still provable.
  - [ ] New coverage in a film spec asserts all three: a save returning `23505` stays saved, an unsave returning zero rows stays unsaved, and an insert returning a non-`23505` error reverts.
  - [ ] `npm run typecheck`, `npm run lint` and the film specs pass.
- **notes:** From `/pr-check` stage 3 on this branch — two confirmed correctness findings against `film-tab.tsx:352` (high) and its comment at `:326` (medium). Root cause: the echo check treats "the database already agrees with you" as a failed write, so a viewer whose teammate already unsaved the point gets stuck showing saved, and every retry repeats the zero-row delete and reverts again. This INVERTS a criterion T6 shipped ("an unsave that matched zero rows reverts") — that is deliberate and owner-approved. Accepted trade-off: treating a zero-row delete as landed gives up UI-level detection of an RLS-refused delete, which is fine because T4's DELETE policy admits anyone who can see the match, the loader only renders points from matches the viewer can see, and `tests/point-bookmarks-db.spec.ts` already proves the refusal at the database level. Do NOT touch the migrations (applied to the live production DB) and do NOT change what any query is scoped by. No spec currently asserts revert-on-zero-rows by name; the mock's header comment is the only place that contract is written down.
