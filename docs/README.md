...
| [`roster-edit-and-people-search.md`](roster-edit-and-people-search.md) | Two designed-but-unbuilt roster changes — editing a player from the row menu (the RPC exists with no caller) and making people findable in the command palette (which searches only `matches` today) — plus why roster filtering was declined. **Point-in-time (2026-08-25)** | You are picking up T1 on `claude/roster-edit-player`, or wondering whether the roster should be filterable |
