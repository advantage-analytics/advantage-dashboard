...
- [ ] **Step 1: `applied-strip.tsx`.** For each `activeFilterEntries(state)`: a 24px pill (`surface-subtle`, hairline, radius 6, 11px, padding `0 4px 0 8px`) with a 16px `x` button (`aria-label={\`Remove ${label}\`}`) that resets that key to its default and clears `viewId`. Then an 11px blue "Clear" → all filters to `EMPTY_VIZ_FILTERS`**keeping`player`**? No — Clear resets `player`too, returning to P1g. Returns`null` when there are no entries so the toolbar drops the divider.
