...
**Dismissed (F4).** Superseded — there is no dismiss (Decisions 5).
...
5. **Collapse** is session state. **No dismiss** (removed 2026-09-16 at the user's request, along with `insight-dismissal.ts`): the summary folds away for a visit but always returns, so F4's dismissed state no longer exists. The card spans the pane in one column with its text at full width (user request, 2026-09-16): the claim at `.text-body` (13px, weight 500, ink-900), the evidence under it at 11px ink-700, then a hairline foot row with the engine credit and Collapse (text reduced at the user's request the same day). `generate-insights` now asks for 2-3 sentence summaries under 350 characters, which set as a one-line claim and one or two evidence lines at 868px of pane. Insight renders on the Statistics view only.
