...
| `Pane`                                          | `min-h-0 min-w-0 flex-1 overflow-y-auto flex flex-col gap-4` with `padding:20px 56px 24px` and `background:var(--surface-page)` — the one place the token is referenced, with a comment naming the conflict with DS principle 6 |
...
**Pane.** `overflow-y-auto; flex-col; gap:16px; padding:20px 56px 24px; background:var(--surface-page)`. Nothing sticky any more (the old tab strip was).
...
1. **Pane is `--surface-page` grey** while cards stay white with hairline+shadow — the frame's choice; conflicts with DS principle 6 ("the report and its rail are `--surface-card`"). One token reference, commented, so reverting is one line.
