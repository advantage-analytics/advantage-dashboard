...
5. **Collapse** is session state. **No dismiss** (removed 2026-09-16 at the user's request, along with `insight-dismissal.ts`): the summary folds away for a visit but always returns, so F4's dismissed state no longer exists. The card spans the pane: from 720px of pane it is two columns, a narrow one (4fr) holding the claim at `.text-title` (16px) over a foot row with the engine credit and Collapse pinned to the column's bottom, and a wide one (8fr) holding the evidence at 12px ink-700 (~75 characters a line); below 720px it stacks as claim, evidence, foot row. Insight renders on the Statistics view only.
