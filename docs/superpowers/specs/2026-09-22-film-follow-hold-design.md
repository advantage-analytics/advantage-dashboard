...
```ts
type PointFocus = { mode: "follow" } | { mode: "held"; pointId: string };
```
...
| Shell point list (`PointList tone="light"`) | The open well (none today: the shell passes no `onSelectShot`); the list's scroll position (keep-in-view off while held). Entered by a row click / Enter / Space only — the light tone installs no hand-scroll hold (T23) | The lit row                                                                                                                                                                                                 |
...
| -------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Click (or Enter/Space) on a point row                                                                                      | Drawer, shell list | `held(thatPointId)` — and the click still seeks (`selectPoint` / `handleSelect` unchanged). Held ≡ playing at that instant, so no pill; the pill appears when the film leaves the point                      |
| Click on a shot row                                                                                                        | Drawer well, card  | `held(stop.point.id)` — the shot's own point; the click still seeks. Same as above                                                                                                                           |
...
| Hand scroll of the drawer list: `wheel`, `touchmove`, or `pointerdown` whose target is the scroller itself (the scrollbar) | Drawer             | `held(displayedPointId)` — the point whose well is open (or, with no well, the lit point). Never from the `scroll` event. The follow effect sets a flag around its own `scrollTo` so its scrolls never count |
...
| A cut change (filters) that removes the held point from `visiblePoints`                                                    | Both               | `follow` — there is no row left to hold. A cut that keeps it changes nothing                                                                                                                                 |
...
| --------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------ | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Held; playing point in the cut, lit row pinned **bottom** | `Now playing · Point 14` + `chevron-down`                  | same as the drawer pill (T23)  | `Now playing: point 14 — follow playback`                 | `held` AND a point is playing AND `playing.id ≠ held.pointId` AND the lit row is not wholly inside the scroller's box                                               |
...
1. **State owner — `film-tab.tsx`, beside `room`** (l.166).
   ...
   exported from `film-timeline.ts` (pure, no React) so `point-list.tsx`, `film-this-point.tsx`
...
pointFocus.pointId : activePointId`. Two stable callbacks: `holdPoint(id)` and
   `followPlayback()`. The tab-leave reset is free (the view unmounts); no effect needed.
2. **The cut-change reset.** An effect on `[filteredPoints, pointFocus]`: if held and no point in
   `filteredPoints` has the held id, `followPlayback()`.
