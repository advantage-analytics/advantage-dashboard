...
| The return affordance (the "Now playing" pill)                                                                      | Drawer, shell list | `follow`. The list scrolls so the playing row sits at the TOP of the box (8px inset) with its well below it (T26)                                                                                                                                                                                       |
| `←` `→` (room: also `↑` `↓`; shell: the same four), the transport chevrons, the card's step buttons                 | Both               | `follow`, then the step from the playing point (`step`, film-fullscreen.tsx l.798; `handleStep`, film-tab.tsx l.308; the key handlers). Stepping means "take me on". A re-follow from held is the same jump as the pill's — the row the step lands on goes to the top of the box (T26)                  |
...
| A cut change (filters) that removes the held point from `visiblePoints`                                             | Both               | `follow` — there is no row left to hold, and the list jumps the playing row to the top of the box, as the pill does (T26). A cut that keeps it changes nothing. A null hold is not reset: it has no row for a cut to drop (T25)                                                                         |
...
**Jump vs. continuous (T26).** A re-follow — any held → follow transition, a null hold's
included: the pill, a step or the transport from held, a click on the playing row from held, a
cut that drops the held point — is a _jump_: the list puts the playing point's row at the top of
the box, `REFOLLOW_JUMP_INSET_PX` (8px, the spacing scale's 8px step) in, clamped at the end of
the list, even when the row is already in view, and always targets the row rather than the lit
shot so the well sits below it. Every other follow run is _continuous_: the minimal scroll that
brings the lit shot or row inside the box, and nothing when it already is. The jump is a window
of `REFOLLOW_JUMP_WINDOW_MS` (800ms) from the transition rather than a single run, because a step
re-follows before its seek lands — the `activePointId` change arrives a tick later, and a one-shot
jump would top-align the old point and leave the new one second. Inside the window both runs
jump and the second wins; the transition is observed inside `PointList`'s keep-in-view effect,
not signalled by a prop, so the drawer and the shell list share it.
...
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Follow-mode keep-in-view       | The effect (l.312–328) computes the target `scrollTop` as today and applies it via `scrollTo` on the scroller; continuous crossings keep the minimal scroll — no lurch per point (T26) | n/a (the card does not scroll to its point)                        | `scrollTo({ top, behavior: "smooth" })` on the list's own `scrollTop` — Chromium eases ~300ms; no custom scroll-jacking; no `scrollIntoView` (l.306–310)                                                                                                                                                                                       |
...
| Re-follow                      | Jump: the playing row to the top of the box, 8px inset, clamped at the end of the list; the well mounts under it (T26)                                                                 | Rows re-key to the playing point                                   | Jump scroll: smooth 300ms / instant under reduced motion; well fold `film-shot-well-open` (250ms) and row stagger `film-shot-row-in` (200ms, 25ms steps) **unchanged**                                                                                                                                                                         |
