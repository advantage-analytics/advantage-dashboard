-- Extend record_splitstep_webhook to persist the vendor's structured error
-- fields (error.code / error.category / error.step) alongside error_message.
--
-- DROP first, not just CREATE OR REPLACE: the new parameter list is a
-- different signature, so OR REPLACE alone would create an overload — and two
-- candidates differing only in defaulted parameters makes every existing
