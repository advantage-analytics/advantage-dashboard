 ...
 * ladder, and without it the field step has nobody to enter.
 *
 * ── The body this route renders ────────────────────────────────────────────
 * `NewTournamentFlow` (`static/new-tournament-flow.tsx`) frames three steps of
 * `matches/new-match-wizard`'s `WizardShell`, the same chrome the new dual and
 * the upload wizard use. It draws `TournamentNameStep`,
 * `TournamentDetailsStep` and `TournamentFieldStep` out of `static/static-tournament-builder.tsx` over that
 ...
 *
 * `defaultSurface` has no cell to fill. The first two steps draw Name, then
 * Starts, Ends, Site and Format, and no surface or host field, so the value travels as
