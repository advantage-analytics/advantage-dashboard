...
export function CourtVisualization({
  ...
  const filteredPoints = useMemo(() => {
    return points.filter((p) => {
      ...
      if (typeSet.size > 0) {
        const isFirst =
          p.firstShotType?.toLowerCase().includes("first") ??
          p.pointScore?.split("-").every((s) => s === "0") ??
          false;
      ...
      if (otherSet.size > 0) {
        const result = classifyResult(p);
        if (otherSet.has("doubleFault") && result !== "doubleFault")
          return false;
      }

      if (spinSet.size > 0) {
        const spin = (
          visualizationType === "return" ? p.secondShotSpin : p.firstShotSpin
        )?.toLowerCase();
    ...
    return filteredPoints.map((point) => {
      const zone =
        (visualizationType === "return"
          ? point.secondShotZone
          : point.firstShotZone) ?? "body";
      const side = getPointSide(point);
      const result = classifyResult(point);

      const landingX =
        visualizationType === "return"
          ? point.secondShotLandingX
          : point.firstShotLandingX;
      const landingY =
        visualizationType === "return"
          ? point.secondShotLandingY
          : point.firstShotLandingY;
      ...
      if (visualizationType === "return") {
        ...
        if (landingX != null && landingY != null) {
          ...
          y = Math.max(
            retFarBaseline + 4,
            Math.min(retFarBaseline + RET_HALF - 4, y),
          );
        } else {
        ...
        const yFraction =
          (n.ly - REAL_SERVICE_LINE_Y) / (REAL_NET_Y - REAL_SERVICE_LINE_Y);
        ...
        if (result === "doubleFault") {
          ...
          y = Math.max(
            PAD_TOP + 4,
            Math.min(baselineYPos + FAR_COURT_H - 4, y),
          );
        } else {
        ...
        if (outsideLeft) {
          ...
          x =
            SINGLES_RIGHT +
            4 +
            ((jx - 0.7) / 0.3) * (COURT_W - SINGLES_RIGHT - 8);
        } else {
          x =
            SINGLES_LEFT + ((jx - 0.3) / 0.4) * (SINGLES_RIGHT - SINGLES_LEFT);
        }
      ...
      >
        <TooltipTrigger asChild>
          <g
            ...
            onMouseLeave={() =>
              setHoveredId((prev) => (prev === dot.point.id ? null : prev))
            }
          ...
          >
            {(() => {
              const isActive =
                hoveredId === dot.point.id || pinnedId === dot.point.id;
              return isAce ? (
                <polygon
                  points={starPoints(
                    dot.x,
        ...
        <TooltipContent
          side="top"
          sideOffset={10}
          className="!w-auto !rounded-xl !border !border-[#F3F3F3] !bg-white !px-0 !py-0 !text-left !shadow-[0px_2px_8px_0px_rgba(0,0,0,0.06)]"
        >
          <div className="flex w-[200px] flex-col">
            ...
            <div className="flex flex-col gap-2 px-3 py-2.5">
              <div className="flex items-baseline justify-between">
                <span className="text-[11px] font-medium tracking-tight text-[#0D0D0D]">
                  {dot.point.gameScore}
                </span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-[#525252]">
                  {dot.point.rallyLength}{" "}
                  {dot.point.rallyLength === 1 ? "shot" : "shots"}
                </span>
              </div>
              {dot.point.description && (
                <span className="text-[10px] leading-tight text-[#888888]">
                  {dot.point.description}
                </span>
              )}
            </div>
  ...
  const svgH = isReturn
    ? RET_SVG_H
    : COURT_H + PAD_TOP + FAR_COURT_H + PAD_BOTTOM;
  const svgLeftPad = isReturn ? RET_LEFT_PAD : NET_EXTEND;
  const svgTotalW = COURT_W + svgLeftPad + NET_EXTEND;

  if (points.length === 0) {
    return (
      <div className="rounded-[14px] border border-[#F3F3F3] bg-white p-5 shadow-[0px_4px_16px_0px_rgba(0,0,0,0.1)]">
        <h2 className="mb-6 text-[10px] font-medium tracking-[2.5px] text-[#AAAAAA] uppercase">
          {title}
        </h2>
        <p className="text-center text-[12px] text-[#525252]">
  ...
  return (
    <motion.div
      className="rounded-[14px] border border-[#F3F3F3] bg-white p-5 shadow-[0px_4px_16px_0px_rgba(0,0,0,0.1)]"
      ...
      <div className="mb-5 flex items-center justify-between">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            <h3 className="text-[10px] font-medium tracking-[2.5px] text-[#AAAAAA] uppercase">
              {title}
            </h3>
            <span
        ...
        <div className="flex items-center gap-3">
            ...
            {VISUALIZATION_TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`h-8 rounded-full px-3.5 text-[11px] font-medium transition-[background-color,color,box-shadow] duration-200 ${
                    isActive
          ...
          <Settings
            size={16}
            className="cursor-pointer text-[#888888] transition-colors duration-200 hover:text-[#525252]"
          />
        </div>
      </div>
          ...
          {isReturn ? (
            <g>
              ...
              <rect
                ...
                y={retFarBaseline}
                width={COURT_W}
                height={RET_FULL + RET_CONTACT_EXTEND}
                fill={COURT_FILL}
              />
              ...
              <line
                x1={SINGLES_LEFT + ZONE_W}
                y1={retFarBaseline}
                x2={SINGLES_LEFT + ZONE_W}
                y2={retNet}
                stroke={COURT_LINE_COLOR_MINOR}
                strokeWidth="2"
                strokeDasharray="6 4"
              />
              <line
                x1={SINGLES_LEFT + 2 * ZONE_W}
                y1={retFarBaseline}
                x2={SINGLES_LEFT + 2 * ZONE_W}
                y2={retNet}
                stroke={COURT_LINE_COLOR_MINOR}
                strokeWidth="2"
                strokeDasharray="6 4"
              ...
              >
                <text
                  x={-12}
                  y={retFarBaseline + RET_DEPTH_ZONE * 0.5}
                  textAnchor="end"
                ...
                <text
                  x={-12}
                  y={retFarBaseline + RET_DEPTH_ZONE * 1.5}
                  textAnchor="end"
                  dominantBaseline="middle"
                >
                  MIDDLE
                ...
                <text
                  x={-12}
                  y={retFarBaseline + RET_DEPTH_ZONE * 2.5}
                  textAnchor="end"
                  dominantBaseline="middle"
                >
                  SHORT
              ...
              <line
                x1={0}
                y1={retFarBaseline + RET_DEPTH_ZONE}
                x2={COURT_W}
                y2={retFarBaseline + RET_DEPTH_ZONE}
                stroke={COURT_LINE_COLOR_MINOR}
                strokeWidth="2"
                strokeDasharray="6 4"
              />
              <line
                x1={0}
                y1={retFarBaseline + 2 * RET_DEPTH_ZONE}
                x2={COURT_W}
                y2={retFarBaseline + 2 * RET_DEPTH_ZONE}
                stroke={COURT_LINE_COLOR_MINOR}
                strokeWidth="2"
                strokeDasharray="6 4"
              />
              ...
              <line
                x1={0}
                y1={retNet + RET_BANNER_H}
                x2={COURT_W}
                y2={retNet + RET_BANNER_H}
                stroke={COURT_LINE_COLOR}
                strokeWidth="2"
              />
              <line
                x1={0}
                y1={retNet + 2 * RET_BANNER_H}
                x2={COURT_W}
                y2={retNet + 2 * RET_BANNER_H}
                stroke={COURT_LINE_COLOR}
                strokeWidth="2"
              />
              <text
              ...
              >
                <text
                  x={-12}
                  y={retNearBaseline + RET_CONTACT_ZONE * 0.5}
                  textAnchor="end"
                  dominantBaseline="middle"
                >
                  INSIDE
                ...
                <text
                  x={-12}
                  y={retNearBaseline + RET_CONTACT_ZONE * 1.5}
                  textAnchor="end"
                  dominantBaseline="middle"
                >
                  NEUTRAL
                ...
                <text
                  x={-12}
                  y={retNearBaseline + RET_CONTACT_ZONE * 2.5}
                  textAnchor="end"
                  dominantBaseline="middle"
                >
                  FAR
              ...
              <line
                x1={0}
                y1={retNearBaseline + RET_CONTACT_ZONE}
                x2={COURT_W}
                y2={retNearBaseline + RET_CONTACT_ZONE}
                stroke={COURT_LINE_COLOR_MINOR}
                strokeWidth="2"
                strokeDasharray="6 4"
              />
              <line
                x1={0}
                y1={retNearBaseline + 2 * RET_CONTACT_ZONE}
                x2={COURT_W}
                y2={retNearBaseline + 2 * RET_CONTACT_ZONE}
                stroke={COURT_LINE_COLOR_MINOR}
                strokeWidth="2"
                strokeDasharray="6 4"
              />
            ...
            <g>
              ...
              >
                ...
                <text
                  x={SINGLES_LEFT + ZONE_W + ZONE_W / 2}
                  y={serviceLineY - 12}
                  textAnchor="middle"
                >
                  BODY
                ...
                <text
                  x={SINGLES_LEFT + 2 * ZONE_W + ZONE_W / 2}
                  y={serviceLineY - 12}
                  textAnchor="middle"
                >
                  T
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-5">
        <div className="flex items-center gap-1.5">
          <div
            ...
            style={{ backgroundColor: DOT_COLORS.won }}
          />
        ...
        <div className="flex items-center gap-1.5">
          <div
            className="h-2.5 w-2.5 rounded-full"
            style={{ backgroundColor: DOT_COLORS.lost }}
          />
        ...
        {!isReturn && (
          <div className="flex items-center gap-1.5">
            <div
              className="h-2.5 w-2.5 rounded-full"
              style={{ backgroundColor: DOT_COLORS.doubleFault }}
            />
