...
const TAG_DEFS: TagDef[] = [
  ...
  { key: "pointScores", prefix: "ps", label: (v) => v },
  {
    key: "servePlayers",
    prefix: "sp",
    label: (v, p1, p2) => `Serve: ${v === "player1" ? p1 : p2}`,
  },
  ...
  { key: "serveZones", prefix: "sz", label: (v) => `Serve: ${v}` },
  {
    key: "returnPlayers",
    prefix: "rp",
    label: (v, p1, p2) => `Return: ${v === "player1" ? p1 : p2}`,
  },
  ...
  { key: "returnContacts", prefix: "rc", label: (v) => `Contact: ${v}` },
  {
    key: "resultPlayers",
    prefix: "rep",
    label: (v, p1, p2) => `Result: ${v === "player1" ? p1 : p2}`,
  },
  ...
  { key: "resultOutcomes", prefix: "reo", label: (v) => v },
  {
    key: "customPlayers",
    prefix: "cp",
    label: (v, p1, p2) => `Custom: ${v === "player1" ? p1 : p2}`,
  },
    ...
    case "score":
      return [
        ...
        },
        {
          label: "Points",
          key: "pointScores",
          options: POINT_SCORES.map((s) => ({ value: s, label: s })),
...
export function VideoFilterBar({
  ...
  const activeTags =
    totalActive > 0
      ? getActiveTags(filters, onFiltersChange, player1Name, player2Name)
      : [];

  return (
    <div className="flex flex-col gap-3">
      {/* Filter trigger bar */}
      <div className="flex h-[52px] items-center gap-2 rounded-[14px] border border-[#F3F3F3] bg-white px-4 shadow-[0px_2px_8px_0px_rgba(0,0,0,0.06)]">
        <span className="mr-1 shrink-0 text-[10px] font-medium tracking-[2px] text-[#AAAAAA] uppercase">
          Filters
        </span>
          ...
          return (
            <Popover key={cat.key}>
              <PopoverTrigger asChild>
                <button
                  className={cn(
                    "inline-flex h-8 items-center gap-1.5 rounded-full px-3.5 text-[11px] font-medium whitespace-nowrap",
                ...
                >
                  {cat.label}
                  {count > 0 && (
                    <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-[#3B82F6] text-[9px] font-medium text-white">
                      {count}
                    </span>
                  )}
              ...
              <PopoverContent className="w-auto max-w-[480px] p-5">
                <div className="flex max-w-[440px] flex-col gap-4">
                  {pills.map((pill) => (
                    <FilterPills
                      ...
                      selected={(filters[pill.key] as (string | number)[]).map(
                        String,
                      )}
                      onChange={(sel) =>
                        onFiltersChange({
                          ...filters,
                          [pill.key]: NUMBER_KEYS.has(pill.key)
                            ? sel.map(Number)
                            : sel,
                        })
                      }
        ...
        {totalActive > 0 && (
          <button
            ...
            className="text-[11px] font-medium whitespace-nowrap text-[#525252] transition-colors duration-200 hover:text-[#E51837]"
          >
            Clear all
          </button>
        )}
            ...
            <div className="flex flex-wrap gap-1.5">
              {activeTags.map((tag) => (
                <motion.button
                  ...
                  className="group inline-flex h-6 items-center gap-1 rounded-full bg-[#F5F5F5] px-2.5 text-[10px] font-medium text-[#525252] transition-colors duration-150 hover:bg-[rgba(229,24,55,0.08)] hover:text-[#E51837]"
                >
                  {tag.label}
                  <X className="h-2.5 w-2.5 opacity-40 transition-opacity group-hover:opacity-100" />
                </motion.button>
              ))}
            </div>
          </motion.div>
