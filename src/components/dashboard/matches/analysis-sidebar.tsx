...
export function AnalysisSidebar({
  ...
  return (
    <motion.div
      className="w-[320px] overflow-hidden rounded-[14px] border border-[#F3F3F3] bg-white shadow-[0px_4px_16px_0px_rgba(0,0,0,0.1)]"
    ...
    >
      ...
      <div className="border-b border-[var(--border-hairline)] p-5">
        <div className="mb-1 flex items-center justify-between">
          ...
          >
            <CircleDot className="h-2.5 w-2.5 text-[#888888]" />
      ...
      {p1 && p2 && (
        <div className="p-5">
          ...
          <div className="flex flex-col divide-y divide-[var(--border-hairline)]">
            ...
            />
            <StatRow
              label={`${match.player1.name.split(" ")[0]} Winners`}
              value={p1.winners ?? "—"}
            />
            <StatRow
              label={`${match.player2.name.split(" ")[0]} Winners`}
              value={p2.winners ?? "—"}
            />
            <StatRow
              label={`${match.player1.name.split(" ")[0]} UE`}
              value={p1.unforcedErrors ?? "—"}
            />
            <StatRow
              label={`${match.player2.name.split(" ")[0]} UE`}
              value={p2.unforcedErrors ?? "—"}
            />
          </div>
        </div>
      )}
