    ...
    if (resultChoice !== "played") {
      startTransition(async () => {
        const result = await setOutcome({
          entryId,
          round,
          outcome: outcomeFromResultChoice(resultChoice),
        });
        if ("error" in result) {
            ...
            <div className="flex items-center gap-3">
              {Array.from({ length: SET_COUNT }).map((_, index) => (
                <div key={index} className="flex gap-1">
                  <SetCell
                    label={`Our games, set ${index + 1}`}
                    value={ours[index]}
                    onChange={(value) => setOurs(replace(ours, index, value))}
                  />
                  <SetCell
                    label={`Opponent games, set ${index + 1}`}
                    value={theirs[index]}
                    onChange={(value) =>
                      setTheirs(replace(theirs, index, value))
                    }
                  />
                  <TiebreakCell
...
function SetCell({
  ...
  return (
    <input
      ...
      className="tabular h-[30px] w-[26px] rounded-[6px] border border-[#EAECF0] bg-white text-center text-[13px] text-[#0D0D0D] outline-none placeholder:text-[#CCCCCC]"
    />
  );
}
...
function TiebreakCell({
  ...
  return (
    <input
      ...
      onChange={(event) => onChange(event.target.value.replace(/[^0-9]/g, ""))}
      className="tabular h-[30px] w-[26px] rounded-[6px] border border-[#EAECF0] bg-white text-center text-[11px] text-[#525252] outline-none"
    />
  );
}
