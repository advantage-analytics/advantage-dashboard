...
export function StatSummaryStrip({ data, trends }: Props) {
  ...
  const TABS: Record<Tab, StatItem[]> = {
    serve: [
      ...
      {
        label: "DFs",
        value: fmt(data.avgDoubleFaults),
        trend: trends.doubleFaults,
      },
      {
        label: "1st In",
        value: fmt(data.avgFirstServePct, "%"),
        trend: trends.firstServePct,
      },
      {
        label: "1st Won",
        value: fmt(data.avgFirstServeWonPct, "%"),
        trend: trends.firstServeWonPct,
      },
      {
        label: "2nd Won",
        value: fmt(data.avgSecondServeWonPct, "%"),
        trend: trends.secondServeWonPct,
      },
      {
        label: "BP Saved",
        value: fmt(data.avgBreakPointsSavedPct, "%"),
        trend: trends.breakPointsSavedPct,
      },
      {
        label: "Svc Games",
        value: fmt(data.avgServiceGamesWonPct, "%"),
        trend: trends.serviceGamesWonPct,
      },
    ],
    return: [
      {
        label: "BP Conv",
        value: fmt(data.avgBreakPointsConvertedPct, "%"),
        trend: trends.breakPointsConvertedPct,
      },
      {
        label: "1st Ret Won",
        value: fmt(data.avgFirstReturnWonPct, "%"),
        trend: trends.firstReturnWonPct,
      },
      {
        label: "2nd Ret Won",
        value: fmt(data.avgSecondReturnWonPct, "%"),
        trend: trends.secondReturnWonPct,
      },
      {
        label: "Ret Games",
        value: fmt(data.avgReturnGamesWonPct, "%"),
        trend: trends.returnGamesWonPct,
      },
    ],
    other: [
      ...
      {
        label: "Errors",
        value: fmt(data.avgUnforcedErrors),
        trend: trends.unforcedErrors,
      },
      {
        label: "Net Pts",
        value: fmt(data.avgNetPointsWonPct, "%"),
        trend: trends.netPointsWonPct,
      },
      {
        label: "Short",
        value: fmt(data.shortRallyWonPct, "%"),
        trend: trends.shortRallyWonPct,
      },
      {
        label: "Medium",
        value: fmt(data.mediumRallyWonPct, "%"),
        trend: trends.mediumRallyWonPct,
      },
      {
        label: "Long",
        value: fmt(data.longRallyWonPct, "%"),
        trend: trends.longRallyWonPct,
      },
      {
        label: "Total Pts",
        value: fmt(data.avgTotalPointsWonPct, "%"),
        trend: trends.totalPointsWonPct,
      },
    ],
  };
