...
export function DurationProfile({ matches }: Props) {
  ...
  const { boxes, globalMin, globalMax, recentDots } = useMemo(() => {
    const withDuration = matches.filter(
      (m) => m.durationSeconds != null && m.durationSeconds > 0,
    );
    const wins = withDuration
      .filter((m) => m.isWin)
      .map((m) => m.durationSeconds! / 60)
      .sort((a, b) => a - b);
    const losses = withDuration
      .filter((m) => !m.isWin)
      .map((m) => m.durationSeconds! / 60)
      .sort((a, b) => a - b);
            ...
            <g key={box.label}>
              ...
              <motion.line
                ...
                transition={
                  shouldReduceMotion
                    ? { duration: 0 }
                    : { duration: 0.5, delay: i * 0.15, ease: EASE_CURVE }
                }
              />
              ...
              <motion.rect
                ...
                initial={
                  shouldReduceMotion
                    ? { x: toX(box.q1), width: toX(box.q3) - toX(box.q1) }
                    : { x: toX(box.median), width: 0 }
                }
                animate={{ x: toX(box.q1), width: toX(box.q3) - toX(box.q1) }}
                transition={
                  shouldReduceMotion
                    ? { duration: 0 }
                    : { duration: 0.5, delay: i * 0.15 + 0.1, ease: EASE_CURVE }
                }
              />
            ...
            {recentDots.map((dot, i) => (
              <motion.circle
                ...
                transition={
                  shouldReduceMotion
                    ? { duration: 0 }
                    : { duration: 0.3, delay: 0.6 + i * 0.06, ease: EASE_CURVE }
                }
              />
            ))}
          </g>
