...
test.describe("daysUntilReset", () => {
  test("counts whole days to the first of the next month, in UTC", () => {
    expect(daysUntilReset("2026-09-01", new Date("2026-09-07T03:00:00Z"))).toBe(
      24,
    );
    expect(daysUntilReset("2026-09-01", new Date("2026-09-30T23:59:00Z"))).toBe(
      1,
    );
    expect(daysUntilReset("2026-12-01", new Date("2026-12-01T00:00:00Z"))).toBe(
      31,
    );
  });

  test("is zero on and after the reset, never negative", () => {
    expect(daysUntilReset("2026-09-01", new Date("2026-10-01T00:00:00Z"))).toBe(
      0,
    );
    expect(daysUntilReset("2026-09-01", new Date("2026-10-15T00:00:00Z"))).toBe(
      0,
    );
  });
});
