...
function dual(): EventDetail {
  ...
  return {
    ...
    entries: results.map(([slot, winner], index) =>
      entry(slot, slot.startsWith("S") ? "singles" : "doubles", index, [
        match(`m-${slot}`, winner),
      ]),
    ),
  };
 ...
 */
const DUAL = dual();
const ON_DUAL = new Map(
  DUAL.entries.map((e) => [e.id, { event: EVENT, entry: e }]),
);
const SEASON = [
  {
    ...seasonMatch("m-1", "2026-03-14T00:00:00.000Z"),
    event_entry_id: "entry-S1",
  },
  {
    ...seasonMatch("m-2", "2026-03-17T00:00:00.000Z"),
    event_entry_id: "entry-S2",
  },
  {
    ...seasonMatch("m-3", "2026-03-20T00:00:00.000Z"),
    event_entry_id: "entry-S3",
  },
];
...
test.describe("teamSeasonKpis · the strip over a full read", () => {
  ...
  test("a match off the dual lineups is analyzed but not averaged", () => {
    ...
    const season = [
      ...
      {
        ...seasonMatch("m-3", "2026-03-20T00:00:00.000Z"),
        event_entry_id: tournamentEntry.id,
      },
    ];
    const { kpis } = teamSeasonKpis(
      season,
