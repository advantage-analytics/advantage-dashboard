...
test.describe("setScopeQuery", () => {
  ...
  test("every other parameter survives a scope change", () => {
    ...
    expect(setScopeQuery(new URLSearchParams("tab=shots"), 3)).toBe(
      "tab=shots&set=3",
    );
    expect(setScopeQuery(new URLSearchParams("tab=film&set=1"), 2)).toBe(
      "tab=film&set=2",
    );
    expect(setScopeQuery(new URLSearchParams("tab=film&set=1"), null)).toBe(
      "tab=film",
    );
  });
