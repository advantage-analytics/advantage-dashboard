...
function event(id: string, kind: EventKind, startsOn: string): ProgramEvent {
  return {
    id,
    programId: "prog",
    kind,
...
function card(
  ...
  return outcomes.map((outcome, index) =>
    entry(`${eventId}-${slots[index]}`, {
      ...
      matches: outcome ? [match(`${eventId}-m${index}`, outcome, status)] : [],
    }),
  );
}
...
function schedule(...items: [ProgramEvent, EventEntry[]][]): ProgramSchedule {
...
test.describe("seasonSummaryFrom · the dual record and its marks", () => {
  test("a season with no events at all is zeroes, not an absence", () => {
    ...
    expect(
      seasonSummaryFrom({ events: [], entriesByEvent: new Map() }),
    ).toEqual({
      form: [],
...
test.describe("seasonSummaryFrom · lines analyzed over lines total", () => {
  ...
  test("all analyzed is n of n", () => {
    const summary = seasonSummaryFrom(
      schedule([
        event("a", "dual", "2026-01-10"),
        card("a", SWEPT, "completed"),
      ]),
    );
...
test.describe("lineCoverageFrom · one event", () => {
  ...
  test("an entry with several matches owes one line per match", () => {
    const coverage = lineCoverageFrom([
      entry("a", {
        matches: [
          match("m1", "us", "completed"),
          match("m2", "us", "processing"),
        ],
      }),
    ]);
    expect(coverage).toEqual({ analyzed: 1, total: 2 });
