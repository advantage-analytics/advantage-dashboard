...
function dual(id: string, startsOn: string, name = `Opponent ${id}`): ProgramEvent {
  return {
    id,
    programId: 'p-1',
    kind: 'dual',
...
function line(
  ...
  return {
    ...
    matches:
      result === 'us' || result === 'them' ? [match(`m-${slot}-${result}`, result)] : [],
  };
}
...
test('cells follow lineWon per court, and the row record sums them', () => {
  ...
  const bySlot = Object.fromEntries(record.rows.map((r) => [r.slot, { ...r, cells: r.cells.map((c) => c.result) }]));
  ...
  expect(s1).toMatchObject({ ours: 'Our Player', theirs: 'Their Player', forfeit: null });
  expect(s1.sets.map((set) => `${set.player1}-${set.player2}`)).toEqual(['6-3', '6-4']);
...
test('columns are chronological whatever order the schedule arrives in', () => {
  ...
  const events = [dual('e-3', '2026-04-11'), dual('e-1', '2026-03-14'), dual('e-2', '2026-03-28')];
...
test('the window keeps the most recent duals and reports the season total', () => {
  const events = Array.from({ length: COURT_RECORD_WINDOW + 3 }, (_, i) =>
    dual(`e-${i}`, `2026-0${1 + Math.floor(i / 9)}-${String(1 + (i % 9)).padStart(2, '0')}`),
  );
  const entries = new Map(events.map((e) => [e.id, [line('S1', 'us')]]));
