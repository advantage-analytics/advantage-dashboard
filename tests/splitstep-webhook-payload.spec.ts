...
const STROKES =
  "https://vendor.blob.core.windows.net/out/job/strokes.json?sig=a";
const PLAYERS =
  "https://vendor.blob.core.windows.net/out/job/players.json?sig=b";
const TRAJECTORIES =
  "https://vendor.blob.core.windows.net/out/job/trajectories.json?sig=c";
const TRIMMED =
  "https://vendor.blob.core.windows.net/out/job/trimmed.mp4?sig=d";
...
test.describe("parseWebhookPayload", () => {
  ...
  test("job_failed: the structured error object is read, not the top-level message", () => {
    ...
    expect(out.errorMessage).toBe(
      "An unexpected error occurred while processing the job.",
    );
    expect(out.strokesUrl).toBeNull();
  });
