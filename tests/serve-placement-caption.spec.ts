...
test.describe("readCourt", () => {
  test("finds the leading zone and the runner-up, ties in bar order", () => {
    ...
    expect(readCourt([0, 0, 0])).toMatchObject({
      total: 0,
      pcts: [0, 0, 0],
      topPct: 0,
    });
...
test.describe("servePlacementCaption", () => {
  test("stays off while either court is too thin to read", () => {
    expect(
      servePlacementCaption({
        deuce: readCourt([2, 0, 0]),
        ad: readCourt([10, 3, 2]),
      }),
    ).toBeNull();
    expect(
      servePlacementCaption({
        deuce: readCourt([10, 3, 2]),
        ad: readCourt([1, 1, 0]),
      }),
    ).toBeNull();
  });
  ...
  test("both courts, one address, same zone", () => {
    expect(
      servePlacementCaption({
        deuce: readCourt([31, 11, 6]),
        ad: readCourt([26, 8, 6]),
      }),
    ).toBe("Both courts go to the T: two thirds of first serves or more.");
  });
  ...
  test("both courts, one address, different zones", () => {
    ...
    ).toBe("Deuce goes to the T, ad goes wide — one address each.");
  });
  ...
  test("neither is an address but one leans", () => {
    expect(
      servePlacementCaption({
        deuce: readCourt([22, 10, 8]),
        ad: readCourt([14, 13, 13]),
      }),
    ).toBe("Deuce leans to the T at 55%; the ad court is closer to even.");
  });

  test("neither is an address and neither leans", () => {
    expect(
      servePlacementCaption({
        deuce: readCourt([16, 12, 12]),
        ad: readCourt([12, 11, 17]),
      }),
    ).toBe(
