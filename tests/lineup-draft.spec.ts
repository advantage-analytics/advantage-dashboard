...
test("sequenceFrom puts the ranked first, then the sentinel, then the bench", () => {
  expect(sequenceFrom(roster, { sentinel: "always" })).toEqual([
    "a",
    "b",
    "c",
    BENCH,
  ...
  expect(sequenceFrom(roster, { sentinel: "if-needed" })).toEqual([
    "a",
    "b",
    "c",
    BENCH,
...
test("with nobody benched, only the editor draws the sentinel", () => {
  ...
  expect(sequenceFrom(full, { sentinel: "always" })).toEqual([
    "a",
    "b",
    "c",
    BENCH,
  ]);
  expect(sequenceFrom(full, { sentinel: "if-needed" })).toEqual([
    "a",
    "b",
    "c",
  ]);
...
test("a sequence built from a roster is not a change to it", () => {
  expect(
    lineupChanged(sequenceFrom(roster, { sentinel: "always" }), roster),
  ).toBe(false);
});
...
test.describe("settledSequence — the saved draft as the resting table draws it", () => {
  ...
  test("keeps the sentinel when somebody is under it", () => {
    expect(settledSequence(["a", "b", BENCH, "c"])).toEqual([
      "a",
      "b",
      BENCH,
      "c",
