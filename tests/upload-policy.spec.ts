...
test.describe("canUploadForProgram", () => {
  for (const [policy, byRole] of Object.entries(TABLE) as [
    UploadPolicy,
    Record<ProgramRole, boolean>,
  ][]) {
    for (const [role, allowed] of Object.entries(byRole) as [
      ProgramRole,
      boolean,
    ][]) {
  ...
  test("a player's own row still narrows them under everyone, and nobody else", () => {
    expect(canUploadForProgram(workspace("player", "everyone", false))).toBe(
      false,
    );
    expect(canUploadForProgram(workspace("coach", "everyone", false))).toBe(
      true,
    );
...
test.describe("explainVideoRefusal", () => {
  ...
  test("tells staff the policy and who can widen it", () => {
    expect(explainVideoRefusal(workspace("staff", "owner_coaches"))).toMatch(
      /owner and coaches/,
    );
    expect(explainVideoRefusal(workspace("coach", "owner"))).toMatch(
      /The owner can widen it/,
    );
  });

  test("tells a player which of the two flags stopped them", () => {
    expect(explainVideoRefusal(workspace("player", "staff"))).toMatch(
      /all staff/,
    );
    expect(explainVideoRefusal(workspace("player", "everyone", false))).toMatch(
      /Can send video/,
    );
  });
});
