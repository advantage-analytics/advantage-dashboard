...
import { compareEntryOrder, courtIndex, DUAL_SLOT_ORDER } from '@/lib/schedule/courts';
...
test.describe('dual court order', () => {
  ...
  test('duals sort by court even when the stored positions collide', () => {
    ...
    expect(collided.sort(compareEntryOrder).map((r) => r.slot)).toEqual(['S1', 'S2']);
  });

  test('duals sort by court even when the stored positions are reversed', () => {
    const reversed = [row('D1', 0), row('S3', 8), row('S1', 4)];
    expect(reversed.sort(compareEntryOrder).map((r) => r.slot)).toEqual(['S1', 'S3', 'D1']);
  });
  ...
  test('a tournament entry has no slot and keeps its stored order', () => {
    ...
    expect(draw.sort(compareEntryOrder).map((r) => r.position)).toEqual([0, 1, 2]);
  });
});
