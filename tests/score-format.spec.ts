...
test.describe("tiebreakOf on every other shape", () => {
  ...
  test("no zero-fill shape in the census renders", () => {
    ...
    for (const [games1, games2] of zeroFill) {
      expect(
        tiebreakOf(set(games1, games2, 0, 0)),
        `${games1}-${games2}`,
      ).toBeNull();
    }
  });
...
test.describe("playedSets trims trailing 0-0 sets", () => {
  test("a phantom third set is dropped", () => {
    expect(
      playedSets([set(6, 4, 0, 0), set(0, 0, 0, 0), set(0, 0, 0, 0)]),
    ).toEqual([set(6, 4, 0, 0)]);
  });

  test("a real two-setter with a phantom third is dropped", () => {
    expect(
      playedSets([set(6, 4, 0, 0), set(7, 6, 0, 0), set(0, 0, 0, 0)]),
    ).toEqual([set(6, 4, 0, 0), set(7, 6, 0, 0)]);
  });
