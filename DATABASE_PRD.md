...
**Fix:**
- **`buildMatchData()`** (`components/dashboard/matches/new-match-wizard/utils.ts`) now combines `formData.date` and `formData.time` into a full ISO datetime string: `"2024-01-15T14:30:00"`. Falls back to date-only if time is not provided.
