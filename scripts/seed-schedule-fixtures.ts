    ...
    lines: emptyDualLines("dual-partial").map((line): LineSpec => {
      if (line.slot === "S1") {
        return {
          ...
          opponents: ["R. Duval"],
          matches: [
            { round: "S1", score: { player1: [6, 7], player2: [2, 5] } },
          ],
        };
      }
      if (line.slot === "S2") {
        return {
          ...
          opponents: ["K. Mensah"],
          matches: [
            { round: "S2", score: { player1: [4, 3], player2: [6, 6] } },
          ],
        };
      }
      return line;
    ...
    lines: [
      {
        ...
        players: ["Clajerson Gimena"],
        ownPlayer: true,
        opponents: ["S. Virtanen"],
        opponentSchool: "Placeholder College",
        matches: [
          {
            round: "R32",
      ...
      },
      {
        ...
        players: ["Theo Nakamura"],
        opponents: ["B. Silva"],
        opponentSchool: "Fixture Tech",
        matches: [
          {
            round: "Q1",
            score: { player1: [6, 6], player2: [4, 4] },
            opponents: ["B. Silva"],
          },
        ],
      },
...
async function main() {
  ...
  if (programError || !program) {
    throw new Error(
      `Target program ${PROGRAM_ID} not found: ${programError?.message}`,
    );
  }
  ...
  const events = await count("program_events", (q: any) =>
    q.eq("program_id", PROGRAM_ID),
  );
  const entries = await count("program_event_entries", (q: any) =>
    q.eq("program_id", PROGRAM_ID),
  );
  const matches = await count("matches", (q: any) =>
    q.eq("program_id", PROGRAM_ID).not("event_entry_id", "is", null),
  );
  const strayEvents = await count("program_events", (q: any) =>
    q.neq("program_id", PROGRAM_ID),
  );
  const strayEntries = await count("program_event_entries", (q: any) =>
    q.neq("program_id", PROGRAM_ID),
  );
  ...
  console.log(
    `  rows under any OTHER program_id: ${strayEvents} events, ${strayEntries} entries`,
  );
  ...
  for (const row of formats ?? []) {
    ...
    if (typeof ad !== "boolean") {
      throw new Error(
        `event "${row.name}" has non-boolean ad_scoring: ${JSON.stringify(ad)}`,
      );
    }
  }
