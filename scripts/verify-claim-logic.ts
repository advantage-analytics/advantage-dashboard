...
check(
  "athletics.wisc.edu",
  registrableDomain("athletics.wisc.edu"),
  "wisc.edu",
);
...
for (const bad of [
  ...
  check(
    `normalizeHost(${JSON.stringify(bad)})`,
    normalizeHost(bad as string),
    "",
  );
}
...
check(
  ...
  matchesListed("utk.edu.example.com", "utk.edu"),
  false,
);
check("prefix must NOT match", matchesListed("notutk.edu", "utk.edu"), false);
check(
  "suffix-no-dot must NOT match",
  matchesListed("xutk.edu", "utk.edu"),
  false,
);
check(
  "reverse must NOT match",
  matchesListed("utk.edu", "vols.utk.edu"),
  false,
);
check(
  "leading dot must NOT match",
  matchesListed(".utk.edu", "utk.edu"),
  false,
);
...
check(
  "freemail routes to review",
  freemailResult.reason.includes("review"),
  true,
);
...
check(
  "text[] athletics_domains works",
  checkClaimEmail("coach@vols.utk.edu", arr).domainMatched,
  true,
);
// The fourth guard: matched, but .com cannot identify an institution.
check(
  "non-academic match does not skip",
  checkClaimEmail("coach@utsports.com", arr).skipsManualReview,
  false,
);
check(
  "non-academic still counts as matched",
  checkClaimEmail("coach@utsports.com", arr).domainMatched,
  true,
);
check(
  "no recorded domain",
  checkClaimEmail("coach@utk.edu", {
    school_name: "X",
    primary_domain: null,
    athletics_domains: null,
  }).reason,
  "no recorded domain for this program",
);

console.log("nextClaimStatus");
check(
  "verify + match",
  nextClaimStatus("pending_email", {
    type: "verify_email",
    domainMatched: true,
  }),
  "objection_window",
...
check(
  "link expires",
  nextClaimStatus("pending_email", { type: "expire" }),
  "rejected",
);
check(
  "approve routes to window",
  nextClaimStatus("pending_review", { type: "approve" }),
  "objection_window",
);
check(
  "reject",
  nextClaimStatus("pending_review", { type: "reject" }),
  "rejected",
);
check(
  "object during review",
  nextClaimStatus("pending_review", { type: "object" }),
  "objected",
);
check(
  "object during window",
  nextClaimStatus("objection_window", { type: "object" }),
  "objected",
);
check(
  "settle",
  nextClaimStatus("objection_window", { type: "settle" }),
  "approved",
);

console.log("nextClaimStatus — illegal moves return null");
check(
  "cannot approve a pending_email",
  nextClaimStatus("pending_email", { type: "approve" }),
  null,
);
check(
  "cannot settle before verifying",
  nextClaimStatus("pending_email", { type: "settle" }),
  null,
);
check(
  "cannot re-verify after review",
  nextClaimStatus("pending_review", {
    type: "verify_email",
    domainMatched: true,
  }),
  null,
...
check(
  "cannot object after approval",
  nextClaimStatus("approved", { type: "object" }),
  null,
);
check(
  "cannot revive a rejection",
  nextClaimStatus("rejected", { type: "approve" }),
  null,
);
check(
  "cannot revive an objection",
  nextClaimStatus("objected", { type: "settle" }),
  null,
);
for (const terminal of ["approved", "rejected", "objected"] as ClaimStatus[]) {
  check(
    `${terminal} is inert`,
    nextClaimStatus(terminal, { type: "settle" }),
    null,
  );
}
...
check(
  "objection_window is active",
  programStatusFor("objection_window"),
  "active",
);
...
check(
  "objection wins over everything",
  reviewReason({
    domainMatched: true,
    announcedRecipients: 5,
    status: "objected",
  }),
  "Someone objected to this claim",
);
check(
  "unannounced is called out",
  reviewReason({
    domainMatched: false,
...
check(
  "review ttl is a week",
  addHours(t0, REVIEW_TOKEN_TTL_HOURS).toISOString(),
...
check(
  "already used",
  isUsable({ expiresAt: later, consumedAt: earlier }, t0),
  false,
);
check(
  "no expiry fails closed",
  isUsable({ expiresAt: null, consumedAt: null }, t0),
  false,
);
check(
  "garbage fails closed",
  isUsable({ expiresAt: "not-a-date", consumedAt: null }, t0),
  false,
);
...
check(
  "rejects another token",
  tokenMatches(generateToken(), hashToken(token)),
  false,
);
