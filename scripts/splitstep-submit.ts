...
async function main() {
  if (!MATCH.matchId || !MATCH.userId) {
    fail(
      "Edit the MATCH block at the top of this script first (matchId, userId).",
    );
  }
  ...
  console.log(
    JSON.stringify({ ...request, VideoUrl: "<minted at submit>" }, null, 2),
  );
  ...
  if (!SUBMIT) {
    console.log(
      "Dry run — nothing sent. Re-run with --submit once the readout above is right.",
    );
    console.log(
  ...
  console.log(
    `✓ video URL minted, expires ${vendorUrl.expiresAt?.toISOString() ?? "never"}`,
  );
  ...
  try {
    ...
    console.warn(
      "⚠ Response is not JSON — external_job_id could not be extracted.",
    );
  }
  const externalJobId = parsed
    ? parseWebhookPayload(parsed).externalJobId
    : null;
  ...
  console.log(
    `✓ Submitted. job=${job.id} external_job_id=${externalJobId ?? "(none)"}`,
  );
  ...
  console.log(
    "Watch for the vendor fetching the video (= processing actually started):",
  );
  console.log(
