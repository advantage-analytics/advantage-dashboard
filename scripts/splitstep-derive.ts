...
async function main() {
  ...
  if (write) {
    ...
    console.log(
      `wrote ${published.pointsWritten} points and ${published.shotsWritten} shots`,
    );
  ...
  console.log(
    `player1 (by ${r.player1Source ?? "nothing"}, not by name): ${r.player1Label}`,
  );
  if (!r.ok) {
    console.log(
      `UNRECONCILED — written anyway under ACCEPT_UNRECONCILED_FOLD: ${t.reason}`,
    );
  }
  ...
  for (const p of t.points) {
    byType[p.result_type ?? "(none)"] =
      (byType[p.result_type ?? "(none)"] ?? 0) + 1;
    for (const f of p.flags) pointFlags[f] = (pointFlags[f] ?? 0) + 1;
    for (const s of p.shots)
      for (const f of s.flags) shotFlags[f] = (shotFlags[f] ?? 0) + 1;
  }
