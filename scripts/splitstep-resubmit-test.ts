...
async function findQuietMatch(): Promise<TestMatch> {
  ...
  throw new Error(
    "No personal match without a live processing job to attach tests to.",
  );
}
...
async function cleanup(): Promise<void> {
  ...
  try {
    const { resolveAzureStorageConfig, deleteVideoBlob } =
      await import("../src/lib/services/splitstep/video-url");
...
async function main(): Promise<void> {
  ...
  // half-set world.
  const { resubmitJob } =
    await import("../src/lib/services/splitstep/resubmit-job");
  const { reconcileVendorJobs } =
    await import("../src/lib/services/splitstep/reconcile");
  const { resolveAzureStorageConfig, videoContainerClient } =
    await import("../src/lib/services/splitstep/video-url");
  ...
  console.log("2. a failed job whose blob is gone is unrecoverable");
  {
    ...
    if (azure) {
      ...
      check(
        ...
        (await childrenOf(goneId)).length === 0,
      );
    } else {
  ...
  if (!azure) {
    ...
    console.log(
      "3. VIDEO_UNREACHABLE-class failure auto-resubmits exactly once",
    );
    let parentId = "";
    let childId = "";
    {
      ...
      check(
        "child carries the parent link",
        child?.resubmitted_from_job_id === parentId,
      );
      check(
        "child is marked auto_resubmitted",
        child?.auto_resubmitted === true,
      );
      check(
        "child reached queued with a vendor id",
      ...
      check(
        "a fresh, unreleased reservation exists for the child",
        u !== null && u.released === false,
        u,
      );
      check(
        "billed to the uploader's own account, re-derived fresh — not a stale usage row",
        u?.account_id === match.created_by,
        u?.account_id,
      );
    }

    console.log(
      "4. a second identical failure gets NO second automatic attempt",
    );
    {
      await supabase
        .from("processing_jobs")
      ...
      check(
        "still exactly one child of the parent",
        (await childrenOf(parentId)).length === 1,
      );
      check(
        "the surfaced state is the failed child",
        (await jobRow(childId))?.status === "failed",
      );
    }

    console.log(
      "5. a manual retry works where auto stopped — then hits the ceiling",
    );
    {
      ...
      check(
        "manual resubmission of the auto-retried failure succeeded",
        r.ok,
        r,
      );
  ...
  const future = new Date(Date.now() + 40 * 60 * 1000);
  {
    ...
    check(
      "no auto-resubmission for a stale job",
      (await childrenOf(staleId)).length === 0,
    );
  }

  console.log(
    "7. reconciliation — a lost completion surfaces as unrecoverable",
  );
  {
