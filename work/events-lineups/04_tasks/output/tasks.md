# Tasks — events-lineups

Twelve tasks, appended to `.claude/tasks/claude-new-session-c3f1ab.md` — a new
queue file for this branch, created with task-add's header and its `.log.md`
sibling. `git check-ignore` confirms `.gitignore:68`'s `!.claude/tasks/`
re-include holds, so neither file is swallowed.

Numbering starts at **T1**: neither `claude-new-session-c3f1ab.md` nor its log
existed, so no id has ever been used on this branch. The `splitstep-integration`
queue is a different branch's file and its ids are not in scope.

## Routing table

| Task | Title | Model | Needs | From |
|---|---|---|---|---|
| T1 | Build the schedule fixtures module | opus | — | plan step 1 |
| T2 | Rebuild 3b — the event-type chooser | opus | T1 | plan step 2 |
| T3 | Rebuild 7e and 7d — the schedule shell and drawer | opus | T1 | plan step 3 |
| T4 | Rebuild 7c and 4c — the dual widget | opus | T3 | plan step 4 |
| T5 | Rebuild 2c — find the school | opus | T1 | plan step 5 |
| T6 | Rebuild 2b — the master-detail dual builder | opus | T5 | plan step 6 |
| T7 | Rebuild 2d and 2e — the add-opponent popup | opus | T6 | plan step 7 |
| T8 | Rebuild 3c — the tournament builder | opus | T1 | plan step 8 |
| T9 | Label the dormant schedule tree | opus | T2, T3, T5, T8 | plan step 9 |
| T10 | Add the copy-fidelity spec | opus | T2, T4, T7, T8 | plan step 10 |
| T11 | Full-set fidelity pass and gates | fable | T9, T10 | plan step 11 (split) |
| T12 | Write the regression note and the flagged-copy list | opus | T11 | plan step 11 (split) |

T1–T10 map one-to-one onto plan steps 1–10, which were already sized for one
subagent context each. **Plan step 11 is the one split**: the cross-screen
verification sweep and the regression note are different work with different
verdicts, and together they ran to six honest criteria — over task-add's
three-to-five range, which is its signal that a task should be two.

## Routing rationale

Nearly everything is `opus`. Each build task is a multi-file change with clear
criteria and real judgment — artboard fidelity is not mechanical, and the
failure mode is subtle drift a `sonnet` pass would not catch itself making.

Two deliberate departures from the obvious tier:

- **T9 up, not down.** Comment-only edits across eight files read as `sonnet`
  work, but they are eight files of 240–509 lines each and the criterion is
  "nothing but comments changed". task-add's tie-break — when in doubt, route
  one tier up — applies: a wrong-low route here costs a mangled dormant file
  and a re-run.
- **T11 on `fable`.** Cross-cutting verification across all ten artboards,
  with criteria that need interpretation ("every divergence found"). That is
  task-add's `fable` definition rather than standard feature work.

## Deviations from task-add, and why

- **Step 2 (dispatch a Fable planner subagent) was not run.** This session is
  instructed not to call the Agent tool unless asked. task-add's own fallback
  covers it: *"If the planner cannot be dispatched at all … say so and draft
  inline under the same rules rather than failing the add."* The Drafting
  rules, Shape, Route, Dependencies and Numbering rules were applied inline.
- **Step 3 (confirm, then write) is served by the pipeline's stage gate.**
  Stage 04's contract points at task-add as "the format and routing authority"
  and instructs the runner to append and mirror; the pipeline's own review gate
  is the human reading `output/` and re-invoking `/feature-next`. Nothing here
  runs until `/task-next` is invoked by hand.
- **No criterion was fabricated.** Every `done when:` line traces to something
  the plan or brief already made observable. Nothing was padded to reach a
  count, and no plan step was too thin to need a refusal.

## Duplicates

None. This branch's queue was empty, and the `splitstep-integration` queue
covers unrelated work.

---

# The block appended to `.claude/tasks/claude-new-session-c3f1ab.md`
...
Beyond the declared inputs (`../03_plan/output/plan.md`,
`.claude/skills/task-add/SKILL.md`; `references/` was empty):

- `git branch --show-current` and `.claude/tasks/` listing — to resolve the
  queue slug and confirm no queue or log exists for this branch, which fixes
  the numbering at T1.
- `.gitignore` and `git check-ignore` — task-add's step 4 trap: a new queue
  file swallowed by the `.claude/*` rule would lose every task added
  afterwards. Line 68 re-includes `.claude/tasks/`; the check passes.
