# Stage 04 output — tasks appended to `.claude/tasks/claude-match-details-implementation-97c1ee.md`

The queue and its `.log.md` sibling were created this stage (none existed for
this branch), so numbering starts at T1. Drafted inline: this session runs on
Fable, the tier task-add routes planning to, so a planner subagent would add
a round trip and no judgment. Format and routing follow
`.claude/skills/task-add/SKILL.md`; the header text is task-add's. Plan steps
map 1→T1, 2→T2, 3→T3, 4→T4, 5→T5, 6→T6, 7→T7, 8a→T8, 8b→T9, 9→T10, 10→T11,
11→T12.

| Task | Title | Model | Needs |
|---|---|---|---|
| T1 | KPI history loader and its spec | opus | — |
| T2 | Thread kpiHistory through getMatchDetailData and the provider | fable | T1 |
| T3 | Set-scope primitive: hook, helpers, chips, spec | opus | — |
| T4 | Shell surface and tab row with the trailing slot | opus | T3 |
| T5 | KPI strip and the Statistics tab layout | opus | T2, T4 |
| T6 | Head-to-head as a 15-row table | opus | T3 |
| T7 | Performance tracker: Expand, above-label, annotation | opus | T3 |
| T8 | Rally length card | sonnet | T3 |
| T9 | How points ended card | sonnet | T3 |
| T10 | Rail metrics and the insight card; retire the strip | opus | T5 |
| T11 | Flags doc rows for 47f | sonnet | T6, T7, T8, T9, T10 |
| T12 | Gate and visual pass against the 47f frame | fable | T11 |

Routing notes: T2 is one tier up because a wrong you-side id is the silent
attribution failure guardrails §4 exists for; T8/T9 are single-file changes
with exact specs; T12 is judgment work across the whole diff. Browser
behaviour appears in `done when:` only where the task can record it in its
own result — a reviewer holding the diff can verdict every other line.

Also consulted: `.claude/tasks/splitstep-integration.md` (header and block
shape of a live queue).
