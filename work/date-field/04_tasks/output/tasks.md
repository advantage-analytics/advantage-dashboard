...
The block appended verbatim to
`.claude/tasks/claude-dual-match-tournament-designs-26cc2f.md`. Nine plan
steps, nine tasks, one per surface.

Numbering starts at T26 because this branch's first queue reached T25 before
add-event-polish's stage 07 deleted the pair (`c4f896a`). Ids are never reused.

## Routing

| id | title | model | needs |
|---|---|---|---|
| T26 | Add the date libraries and the ISO conversion module | opus | — |
| T27 | Build the `DateField` primitive | fable | T26 |
| T28 | Migrate the statistics from/to dates | sonnet | T27 |
| T29 | Migrate the profile birthdate | sonnet | T27 |
| T30 | Migrate the dual facts Date cell | opus | T27 |
| T31 | Migrate the tournament builder's Starts and Ends | opus | T27 |
| T32 | Migrate the upload wizard's date cell | fable | T27 |
| T33 | Migrate the match-edit dialog's date | opus | T27 |
| T34 | Write the design system's date-field rule | opus | T28, T29, T30, T31, T32, T33 |

T28–T33 are independent of each other in code and could run in any order; each
carries only `needs: T27`. Run them serially anyway — they share a dev server,
and two agents in `package-lock.json` at once buys nothing.
