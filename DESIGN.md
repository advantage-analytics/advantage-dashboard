...
- **Focus:** 2px ring at blue 40% on buttons, links, tabs and pills. **Text fields are the exception** — `<input>`, `<textarea>` and the `Select` trigger take a neutral 1px #E5E5E5 ring (`src/components/ui/input.tsx`), so a form does not spend the accent once per field; underline inputs keep the blue vocabulary, where the rule itself thickens to 2px blue. Today `design-system/focus.css` still hands the blue `--focus-ring` to bare `input, select, textarea` at specificity 0, so a hand-rolled field focuses blue until it sets the neutral classes.
